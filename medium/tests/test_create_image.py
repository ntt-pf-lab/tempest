...
class CreateImageFatTest(FunctionalTest):
    def setUp(self):
        ...
        subprocess.check_call('%s flavor create '
            '--name=small --memory=1024 --cpu=1 --local_gb=1 '
            '--flavor=%d --swap=0' % (self.config.nova.nova_manage_path,
                            self.small_flavor_ref), shell=True)

        self.fat_flavor_ref = 999
        subprocess.check_call('%s flavor create '
            '--name=fat --memory=1024 --cpu=1 --local_gb=2 '
            '--flavor=%d --swap=0' % (self.config.nova.nova_manage_path,
                                    self.fat_flavor_ref), shell=True)

        def flush_flavors():
            subprocess.call('%s flavor delete small --purge' %
                    self.config.nova.nova_manage_path, shell=True)
            subprocess.call('%s flavor delete fat --purge' %
                    self.config.nova.nova_manage_path, shell=True)
